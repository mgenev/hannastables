...
class FUE_Woocommerce {
    ...
    public function __construct() {
        ...
        // settings page
        add_action( 'fue_settings_form', array($this, 'settings_form') );
    ...
    public function email_type_triggers( $type_triggers ) {
        $type_triggers['normal']    = array('first_purchase', 'cart');
    ...
    public function settings_form() {
        ...
        <h3><?php _e('Documentation and Compatibility', 'follow_up_emails'); ?></h3>

        <p><a href="http://docs.woothemes.com/document/automated-follow-up-emails/" target="_blank"><?php _e('Follow-up Emails Documentation', 'follow_up_emails'); ?></a></p>
        <p><?php _e('Follow-up Emails is integrated with the following extensions', 'follow_up_emails'); ?>
        <p><a href="http://www.woothemes.com/products/woocommerce-subscriptions/?utm_source=woocommerce&utm_medium=wordpress-admin&utm_campaign=followup-emails-plugin" target="_blank"><?php _e('WooCommerce Subscriptions', 'follow_up_emails'); ?></a></p>
        <p><a href="http://tri.be/shop/wordpress-wootickets/?utm_source=woocommerce&utm_medium=wordpress-admin&utm_campaign=followup-emails-plugin" target="_blank"><?php _e('WooCommerce Tickets', 'follow_up_emails'); ?></a></p>
        <p><a href="http://www.woothemes.com/products/sensei/?utm_source=woocommerce&utm_medium=wordpress-admin&utm_campaign=followup-emails-plugin" target="_blank"><?php _e('Sensei', 'follow_up_emails'); ?></a></p>
        <p><a href="http://www.woothemes.com/products/woocommerce-points-and-rewards/?utm_source=woocommerce&utm_medium=wordpress-admin&utm_campaign=followup-emails-plugin" target="_blank"><?php _e('WooCommerce Points and Rewards', 'follow_up_emails'); ?></a></p>
        <p><a href="http://wordpress.org/plugins/woocommerce-custom-statuses/?utm_source=woocommerce&utm_medium=wordpress-admin&utm_campaign=followup-emails-plugin" target="_blank"><?php _e('WooCommerce Custom Statuses', 'follow_up_emails'); ?></a></p>
    ...
    function hook_statuses() {
        ...
        if (! isset($order_statuses['errors']) ) {
            foreach ( $order_statuses as $status ) {
                add_action('woocommerce_order_status_'. $status->slug, array($this, 'new_order'), 100);
            }
        }
    }
    ...
    public static function create_email_orders( $triggers, $order_id = '' ) {
        ...
        if ( $order !== false ) {
            ...
            if ( $fue_customer_id ) {
                // first purchase
                /*$emails = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}followup_emails WHERE interval_type = 'first_purchase' ORDER BY priority ASC");
                    ...
                    if ( $emails ) {
                        foreach ( $emails as $email ) {
                            ...
                            if ( $count == 1 ) {
                                ...
                                FUE::queue_email( array('order_id' => $order_id), $email );
                                $num_queued++;
                            }
                        }
                    }
    ...
    public function email_variables( $vars, $email_data, $email_order, $email_row ) {
        ...
        switch ( $email_type ) {
            case 'generic':
                $vars = array_merge($vars, array('{order_number}', '{order_date}', '{order_datetime}', '{customer_first_name}', '{customer_name}', '{customer_email}', '{item_names}', '{item_categories}'));
                break;
            ...
            case 'reminder':
            case 'normal':
                $vars = array_merge($vars, array('{order_number}', '{order_date}', '{order_datetime}', '{customer_first_name}', '{customer_name}', '{customer_email}', '{item_name}', '{item_category}'));
                break;
        }
